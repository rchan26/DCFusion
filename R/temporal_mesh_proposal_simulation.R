...
vanilla_BF_proposal <- function(s,
                                ...
                                sub_posterior_samples) {
  if (t <= s) {
    stop("vanilla_BF_proposal: must have s < t < end_time")
  } else if (end_time <= t) {
    stop("vanilla_BF_proposal: must have s < t < end_time")
  } else if (!is.list(sub_posterior_samples)) {
    stop("vanilla_BF_proposal: sub_posterior_samples must be a list of length C")
  ...
  if (d==1) {
    if (!all(sapply(1:length(sub_posterior_samples), function(i) is.double(sub_posterior_samples[[i]])))) {
      stop("vanilla_BF_proposal: if d==1, sub_posterior_samples[[i]] must be a double for all i=1,...,C")
    }
  } else if (d > 1) {
    ...
    stop("vanilla_BF_proposal: d must be greater than or equal to 1")
  }
  V <- construct_V_vanilla(s, t, end_time, C, d)
  M <- construct_M_vanilla(s, t, end_time, C, d, sub_posterior_samples)
  return(list('M' = M$M,
              'M_list' = M$M_list,
              'V' = V$V,
              'V_inv' = V$V_inv))
