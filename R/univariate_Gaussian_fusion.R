...
#'
#' @export
ea_uniGaussian_DL <- function(N, input_samples, time, mean, sd, beta, precondition) {
...
#'
#' Monte Carlo Fusion for sub-posteriors that are tempered Gaussian distributions
#'
#' @param N number of samples
...
parallel_fusion_uniGaussian <- function(N,
  ...
  varlist <- c(ls(), list("ea_phi_uniGaussian_DL",
...
#'
#' Hierarchical Monte Carlo Fusion with base tempered Gaussians with same mean and standard deviation
#'
...
#'
#' @export
hierarchical_fusion_uniGaussian <- function(N_schedule,
                                            m_schedule,
                                            time_schedule,
                                            base_samples,
                                            L,
  ...
  for (k in ((L-1):1)) {
    ...
    cat('Starting to fuse', m_schedule[k], 'densities of pi^beta, where beta =',
        ...
        file = 'hierarchical_fusion_uniGaussian.txt', append = T)
...
Q_IS_uniGaussian <- function(particle_set,
                             ...
                             seed = NULL,
                             n_cores = parallel::detectCores()) {
  if (!is.vector(means) | (length(means)!=m)) {
...
#'
#' Hierarchical SMC Fusion with base tempered Gaussians with same mean and standard deviation
#'
#' @param N_schedule vector of length (L-1), where N_schedule[l] is the number 
#'                   of samples per node at level l
#' @param m_schedule vector of length (L-1), where m_schedule[l] is the number 
...
#'
#' @export
hierarchical_fusion_SMC_uniGaussian <- function(N_schedule,
                                                m_schedule,
                                                time_schedule,
                                                base_samples,
                                                L,
  ...
  for (k in ((L-1):1)) {
    ...
    cat('Starting to fuse', m_schedule[k], 'densities of pi^beta, where beta =',
        prod(m_schedule[L:(k+1)]), '/', (1/start_beta), 'for level', k, 'with time',
        time_schedule[k], ', which is using', parallel::detectCores(), 'cores\n',
        file = 'hierarchical_fusion_SMC_uniGaussian.txt', append = T)
    cat('There are', n_nodes, 'nodes at this level each giving', N_schedule[k],
        'samples for beta =', prod(m_schedule[L:k]), '/', (1/start_beta),
        '\n', file = 'hierarchical_fusion_SMC_uniGaussian.txt', append = T)
    cat('########################\n', file = 'hierarchical_fusion_SMC_uniGaussian.txt', 
...
#'
#' Progressive SMC Fusion with base tempered Gaussians with same mean and standard deviation
#'
#' @param N_schedule vector of length (L-1), where N_schedule[l] is the number 
#'                   of samples per node at level l
#' @param time_schedule vector of legnth(L-1), where time_schedule[l] is the time 
