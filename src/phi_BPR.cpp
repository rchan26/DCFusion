...
arma::vec log_BPR_gradient(const arma::vec &beta,
  ...
  for (int k=0; k < X.n_cols; ++k) {
    for (int i=0; i < X.n_rows; ++i) {
      gradient.at(k) += count.at(i)*X.at(i,k)*(y_count.at(i)-exp(X_beta.at(i)));
    }
...
arma::mat log_BPR_hessian(const arma::mat &X,
  ...
  for (int i=0; i < X.n_rows; ++i) {
    ...
    for (int j=0; j < X.n_cols; ++j) {
      for (int k=0; k <= j; ++k) {
        hessian.at(j,k) -= count.at(i)*X.at(i,j)*X.at(i,k)*exp_X_beta;
      }
    }
  }
  for (int j=0; j < X.n_cols; ++j) {
...
Rcpp::List ea_phi_BPR_DL_vec(const arma::vec &beta,
  ...
  const arma::vec gradient = log_BPR_gradient(beta,
                                              y_count,
                                              X,
                                              X_beta,
                                              count,
  ...
  const arma::mat hessian = log_BPR_hessian(X, X_beta, count, prior_variances, C);
...
Rcpp::List ea_phi_BPR_DL_matrix(const arma::mat &beta,
  ...
  for (int i=0; i < beta.n_rows; ++i) {
    Rcpp::List phi_eval = ea_phi_BPR_DL_vec(arma::trans(beta.row(i)),
                                            y_count,
                                            X,
                                            count,
                                            prior_means,
...
double spectral_radius_BPR(const arma::vec &beta,
  ...
  const arma::vec X_beta = X * beta;
  return(spectral_radius(Lambda * log_BPR_hessian(X,
                                                  X_beta,
                                                  count,
                                                  prior_variances,
                                                  C)));
...
Rcpp::List obtain_hypercube_centre_BPR(const Rcpp::List &bessel_layers,
  ...
  return(Rcpp::List::create(Named("beta_hat", beta_hat),
                            Named("grad_log_hat", log_BPR_gradient(beta_hat,
                                                       y_count,
                                                       X,
                                                       X_beta,
                                                       count,
...
Rcpp::List spectral_radius_bound_BPR_Z(const int &dim,
  ...
  arma::vec exp_u(V.n_rows, arma::fill::zeros);
  for (int i=0; i < transformed_X.n_rows; ++i) {
    for (int v=0; v < V.n_rows; ++v) {
      exp_u.at(v) = exp(arma::dot(transformed_X.row(i), V.row(v)));
    }
    const double max_exp_u = exp_u.max();
    for (int k=0; k < dim; ++k) {
      for (int l=0; l <= k; ++l) {
        hessian.at(k,l) -= count.at(i)*transformed_X.at(i,k)*transformed_X.at(i,l)*max_exp_u;
      }
    }
  }
  for (int k=0; k < dim; ++k) {
...
Rcpp::List ea_phi_BPR_DL_bounds(const arma::vec &beta_hat,
  ...
  const double dist = maximal_distance_hypercube_to_cv(beta_hat,
                                                       ...
                                                       transform_to_Z,
                                                       true);
  Rcpp::List spectral_radius_bds = spectral_radius_bound_BPR_Z(dim,
                                                               vertices,
                                                               transformed_X,
                                                               count,
                                                               prior_variances,
...
double gamma_NB_BPR(const arma::vec &times,
                    ...
                    const arma::mat &precondition_mat) {
  if (times.size() < 2) {
    stop("gamma_NB_BPR: length of times must be at least 2"); 
  } 
  double sum_phi_eval = 0;
  for (int i=0; i < times.size(); ++i) {
    const arma::vec eval = (x0*(t-times.at(i))+y*(times.at(i)-s))/(t-s);
    Rcpp::List phi = ea_phi_BPR_DL_vec(eval,
                                       y_count,
                                       X,
                                       count,
                                       prior_means,
...
// // [[Rcpp::export]]
// arma::vec log_BPR_gradient_Z(const arma::vec &beta,
//                              const arma::vec &y_count,
//                              const arma::vec &X_beta,
//                              const arma::mat &transformed_X,
//                              const arma::vec &count,
...
//     for (int i=0; i < transformed_X.n_rows; ++i) {
//       gradient.at(k) += count.at(i)*transformed_X.at(i,k)*(y_count.at(i)-exp(X_beta.at(i)));
//     }
...
// // [[Rcpp::export]]
// double BPR_term2_Z(const arma::vec &X_beta,
//                    const arma::mat &transformed_X,
//                    const arma::vec &count,
//                    const arma::vec &prior_variances,
//                    const double &C,
...
//     for (int i=0; i < transformed_X.n_rows; ++i) {
//       const double exp_X_beta = exp(X_beta.at(i));
//       divergence -= count.at(i)*transformed_X.at(i,k)*transformed_X.at(i,k)*exp_X_beta;
//     }
//     for (int j=0; j < transformed_X.n_cols; ++j) {
...
// // [[Rcpp::export]]
// Rcpp::List ea_phi_BPR_DL_vec_Z(const arma::vec &beta,
//                                const arma::vec &y_count,
//                                const arma::mat &X,
//                                const arma::vec &count,
//                                const arma::vec &prior_means,
...
//   const arma::vec gradient = log_BPR_gradient_Z(beta,
//                                                 y_count,
//                                                 X_beta,
//                                                 transformed_X,
//                                                 count,
...
//   const double t2 = BPR_term2_Z(X_beta,
//                                 transformed_X,
//                                 count,
//                                 prior_variances,
//                                 C,
...
// // [[Rcpp::export]]
// double BPR_term2_X(const arma::mat &X,
//                    const arma::vec &X_beta,
//                    const arma::vec &count,
//                    const arma::vec &prior_variances,
//                    const double &C,
