...
arma::mat log_BLR_hessian(const arma::mat &X,
  ...
  for (int i=0; i < X.n_rows; ++i) {
    ...
    const double ratio = count.at(i)*exp_X_beta/((1+exp_X_beta)*(1+exp_X_beta));
...
Rcpp::List spectral_radius_global_bound_BLR_Z(const int &dim,
  ...
  for (int i=0; i < transformed_X.n_rows; ++i) {
    for (int k=0; k < dim; ++k) {
      for (int l=0; l <= k; ++l) {
        hessian.at(k,l) += count.at(i)*std::abs(transformed_X.at(i,k))*std::abs(transformed_X.at(i,l))/4;
      }
    }
  }
  for (int k=0; k < dim; ++k) {
