...
C <- 4
cov_mat <- C*matrix(c(sd[1]^2, 0.9, 0.9, sd[2]^2), nrow = 2, ncol = 2, byrow = T)
diffusion_estimator <- 'NB'

# sampling from the sub-posteriors (target at inverse temperature 1/4)
input_samples <- lapply(1:C, function(i) mvrnormArma(1000000, mu = mean, Sigma = cov_mat))

# sample from true target density
true_samples <- mvrnormArma(1000000, mu = mean, Sigma = cov_mat/C)
...
MCF_standard <- parallel_fusion_SMC_multiGaussian(particles_to_fuse = input_particles_MCF,
                                                  N = nsamples,
                                                  m = C,
                                                  time = fusion_time,
                                                  dim = 2,
...
MCF_generalised <- parallel_fusion_SMC_multiGaussian(particles_to_fuse = input_particles_MCF,
                                                     N = nsamples,
                                                     m = C,
                                                     time = fusion_time,
                                                     dim = 2,
...
BF_standard_n1 <- parallel_GBF_multiGaussian(particles_to_fuse = input_particles_BF_n1,
                                             N = nsamples,
                                             m = C,
...
BF_generalised_n1 <- parallel_GBF_multiGaussian(particles_to_fuse = input_particles_BF_n1,
                                                N = nsamples,
                                                m = C,
                                                time_mesh = time_mesh_BF_n1,
                                                dim = 2,
...
BF_standard_n20 <- parallel_GBF_multiGaussian(particles_to_fuse = input_particles_BF_n20,
                                              N = nsamples,
                                              m = C,
...
BF_generalised_n20 <- parallel_GBF_multiGaussian(particles_to_fuse = input_particles_BF_n20,
                                                 N = nsamples,
                                                 m = C,
                                                 time_mesh = time_mesh_BF_n20,
                                                 dim = 2,
...
save.image('bivariate_Gaussian_bf_example_multi.RData')
