...
collect_results <- function(results, dc, nsamples, dim, marg_means, marg_sds, seed) {
...
for (i in 1:length(nsamples)) {
  ...
  for (rep in 10:number_of_replicates) {
    ...
    input_samples <- lapply(1:C, function(sub) mvrnormArma(N = nsamples[i], mu = mean, Sigma = cov_mat))
    if (dimension==1) {
    ...
    dc_gbf_regular <- bal_binary_GBF_multiGaussian(N_schedule = rep(nsamples[i], 3),
                                                   m_schedule = rep(2, 3),
    ...
    dc_gbf_adaptive <- bal_binary_GBF_multiGaussian(N_schedule = rep(nsamples[i], 3),
                                                    m_schedule = rep(2, 3),
                                                    base_samples = input_samples,
                                                    L = 4,
                                                    dim = dimension,
    ...
    save.image('dimension_study_effect_of_N_replicates.RData')
  }
}
...
##### Paper: IAD #####
plot(x = log(nsamples),
     y = sapply(1:length(nsamples), function(i) {
       mean(log(sapply(1:number_of_replicates, function(rep) dc_gbf$regular[[i]][[rep]]$IAD)))
     }),
     type = 'b', pch = 1, lty = 1, lwd = 3, ylim = c(-4,0), xaxt = 'n', yaxt ='n', xlab = '', ylab = '')
lines(x = log(nsamples),
      y = sapply(1:length(nsamples), function(i) {
        mean(log(sapply(1:number_of_replicates, function(rep) dc_gbf$adaptive[[i]][[rep]]$IAD)))
      }),
      pch = 2, lty = 2, lwd = 3, type = 'b', col = 'red')
for (i in 1:length(nsamples)) {
  IAD <- sapply(1:number_of_replicates, function(rep) dc_gbf$regular[[i]][[rep]]$IAD)
  points(x = rep(nsamples[i], length(IAD)), y = log(IAD), cex = 0.5, pch = 1, lwd = 1.5)
}
for (i in 1:length(nsamples)) {
  IAD <- sapply(1:number_of_replicates, function(rep) dc_gbf$adaptive[[i]][[rep]]$IAD)
  points(x = rep(nsamples[i], length(IAD)), y = log(IAD), cex = 0.5, pch = 2, lwd = 1.5, col = 'red')
}
axis(1, at = log(nsamples), labels = log(nsamples), font = 2, cex = 1.5)
mtext('nsamples', 1, 2.75, font = 2, cex = 1.5)
...
mtext('log(nsamples, 2)', 1, 2.75, font = 2, cex = 1.5)
