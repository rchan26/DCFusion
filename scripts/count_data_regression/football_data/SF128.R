...
data_split_128 <- split_data(scottish_football$data,
                             y_col_index = 19,
                             X_col_index = 1:18,
                             C = C,
                             as_dataframe = F)
...
for (i in 1:128) {
  data_split_128[[i]]$X <- data_split_128[[i]]$X[,-1]
  data_split_128[[i]]$full_data_count <- subset(data_split_128[[i]]$full_data_count, select = -V2)
  data_split_128[[i]]$design_count <- subset(data_split_128[[i]]$design_count, select = -V1)
}
sub_posteriors_128 <- hmc_base_sampler_GLMR(likelihood = 'NB_2',
