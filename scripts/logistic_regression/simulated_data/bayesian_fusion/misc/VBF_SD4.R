...
Poisson_DCMCF_4 <- bal_binary_fusion_SMC_BLR(N_schedule = rep(nsamples, 2),
                                             m_schedule = rep(2, 2),
...
Poisson_DCMCF_4$particles <- resample_particle_y_samples(particle_set = Poisson_DCMCF_4$particles[[1]],
                                                         multivariate = TRUE,
                                                         resampling_method = 'resid',
                                                         seed = seed)
Poisson_DCMCF_4$proposed_samples <- Poisson_DCMCF_4$proposed_samples[[1]]
print(integrated_abs_distance(full_posterior, Poisson_DCMCF_4$particles$y_samples))
compare_samples_bivariate(posteriors = list(full_posterior,
                                            Poisson_DCMCF_4$particles$y_samples,
                                            Poisson_DCMCF_4$proposed_samples),
                          colours = c('black', 'red', 'green'),
                          common_limit = c(-4, 4))

##### NB (Hypercube Centre) #####
print('NB Fusion (hypercube centre)')
NB_DCMCF_4 <- bal_binary_fusion_SMC_BLR(N_schedule = rep(nsamples, 2),
                                        m_schedule = rep(2, 2),
                                        time_schedule = rep(time_choice, 2),
                                        base_samples = sub_posteriors_4,
                                        L = 3,
                                        ...
                                        n_cores = n_cores,
                                        print_progress_iters = 500)
NB_DCMCF_4$particles <- resample_particle_y_samples(particle_set = NB_DCMCF_4$particles[[1]],
                                                    multivariate = TRUE,
                                                    resampling_method = 'resid',
                                                    seed = seed)
NB_DCMCF_4$proposed_samples <- NB_DCMCF_4$proposed_samples[[1]]
print(integrated_abs_distance(full_posterior, NB_DCMCF_4$particles$y_samples))
compare_samples_bivariate(posteriors = list(full_posterior,
                                            NB_DCMCF_4$particles$y_samples,
                                            NB_DCMCF_4$proposed_samples),
                          colours = c('black', 'red', 'green'),
                          common_limit = c(-4, 4))
...
##### Poisson (Hypercube Centre) #####
print('Poisson Fusion (hypercube centre)')
Poisson_VBF_4 <- parallel_vanilla_BF_SMC_BLR(particles_to_fuse = particles_to_fuse,
                                             N = nsamples,
                                             m = 4,
                                             time_mesh = time_mesh,
                                             dim = 5,
                                             ...
                                             n_cores = n_cores,
                                             print_progress_iters = 500)
Poisson_VBF_4$particles <- resample_particle_y_samples(particle_set = Poisson_VBF_4$particles,
                                                       multivariate = TRUE,
                                                       resampling_method = 'resid',
                                                       seed = seed)
...
##### NB (Hypercube Centre) #####
print('NB Fusion (hypercube centre)')
NB_VBF_4 <- parallel_vanilla_BF_SMC_BLR(particles_to_fuse = particles_to_fuse,
                                        N = nsamples,
                                        m = 4,
                                        time_mesh = time_mesh,
                                        dim = 5,
...
integrated_abs_distance(full_posterior, Poisson_DCMCF_4$particles$y_samples)
integrated_abs_distance(full_posterior, NB_DCMCF_4$particles$y_samples)
integrated_abs_distance(full_posterior, Poisson_VBF_4$particles$y_samples)
integrated_abs_distance(full_posterior, NB_VBF_4$particles$y_samples)
