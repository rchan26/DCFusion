...
nsamples <- 100000
ndata <- 5000
time_choice <- c(0.5, 0.5, 0.5, 0.5, 0.5, 0.4, 0.4, 0.4)
...
for (d in 6:length(dim)) {
  ...
  Poisson_hc_16[[d]] <- bal_binary_fusion_SMC_BLR(N_schedule = rep(nsamples, 4),
                                                  m_schedule = rep(2, 4),
                                                  time_schedule = rep(time_choice[d], 4),
  ...
  print('save_progress')
  save.image('SD16_dim_study_TURING_05.RData')
...
plot(x = dim, y = log(sapply(1:length(dim), function(d) sum(unlist(Poisson_hc_16[[d]]$time)))),
     ylim = c(-2, 10),
     xlab = '',
     ylab = '',
...
lines(x = dim, y = log(sapply(1:length(dim), function(d) sum(unlist(NB_hc_16[[d]]$time)))),
      lty = 2, lwd = 3)
