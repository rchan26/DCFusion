...
for (i in 1:length(data_sizes)) {
  ...
  d_BF_generalised <- parallel_GBF_biGaussian(particles_to_fuse = input_particles,
                                              ...
                                              seed = seed*i)
  # save results
  d_results$vanilla[[i]] <- collect_results(d_BF_standard)
  d_results$generalised[[i]] <- collect_results(d_BF_generalised)
  ...
  SH_adaptive_generalised <- parallel_GBF_biGaussian(particles_to_fuse = input_particles,
                                                     ...
                                                     seed = seed*i)
  # save results
  SH_adaptive_results$vanilla[[i]] <- collect_results(SH_adaptive_standard)
  SH_adaptive_results$generalised[[i]] <- collect_results(SH_adaptive_generalised)
}
...
save.image('bf_bivG_dissimilar_means.RData')
