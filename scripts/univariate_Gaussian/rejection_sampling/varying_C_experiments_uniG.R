...
for (i in 1:length(denominator)) {
  ...
  if (denominator[i]==2) {
    ...
    hier_fused <- hierarchical_fusion_uniGaussian(N_schedule = nsamples,
                                                  m_schedule = 2,
                                                  time_schedule = time_choice,
                                                  base_samples = input_samples,
                                                  mean = 0,
    ...
    print('performing preconditioned hierarchical MC fusion')
    hier_fused <- hierarchical_fusion_uniGaussian(N_schedule = rep(nsamples, 2),
                                                  m_schedule = rep(2, 2),
    ...
    print('performing preconditioned hierarchical MC fusion')
    hier_fused <- hierarchical_fusion_uniGaussian(N_schedule = rep(nsamples, 3),
                                                  m_schedule = rep(2, 3),
    ...
    print('performing preconditioned hierarchical MC fusion')
    hier_fused <- hierarchical_fusion_uniGaussian(N_schedule = rep(nsamples, 4),
                                                  m_schedule = rep(2, 4),
...
plot(x = 2:16, y = sapply(1:15, function(i) log(fnj_precondition_results[[i]][[1]])), ylim = c(-1, 8),
