...
plot(x = different_modes, y = sapply(1:length(different_modes), function(i) log(fusion_samples_precondition[[i]]$time)), 
     ylim = c(-1,12), ylab = 'Time Elapsed in log(seconds)', xlab = expression(paste('Value of ', mu)), 
     col = 'black', lwd = 2, xaxt = "n")
lines(x = different_modes, y = sapply(1:length(different_modes), function(i) log(fusion_samples_precondition[[i]]$time)),
      col = 'black', lwd = 2)
axis(1, at=0:8, labels=0:8)
