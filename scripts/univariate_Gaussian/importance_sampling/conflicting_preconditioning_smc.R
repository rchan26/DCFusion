...
new_parameters <- function(mu1, mu2, sd1, sd2) {
  mu <- (sd1*sd1*mu2 + sd2*sd2*mu1) / (sd1*sd1 + sd2*sd2)
  std_dv <- sqrt(1 / ((1/(sd1*sd1)) + (1/(sd2*sd2))))
...
lines(density(all_in_one$samples))
...
particles_to_fuse <- list(pi1pi2$particles[[1]], 
                          initialise_particle_sets(sub_post_samples = sub_post_samples[[3]], 
                                                   multivariate = FALSE))
fused_method_1 <- parallel_fusion_SMC_uniGaussian(particles_to_fuse = particles_to_fuse,
                                                  ...
                                                  betas = rep(1, 2), 
                                                  precondition_values = c(pi1pi2$precondition_values[[1]], var(sub_post_samples[[3]])),
                                                  ESS_threshold = 0,
                                                  seed = seed)
...
particles_to_fuse <- list(pi1pi3$particles[[1]], 
                          initialise_particle_sets(sub_post_samples = sub_post_samples[[2]], 
                                                   multivariate = FALSE))
...
particles_to_fuse <- list(pi2pi3$particles[[1]], 
                          initialise_particle_sets(sub_post_samples = sub_post_samples[[1]], 
                                                   multivariate = FALSE))
